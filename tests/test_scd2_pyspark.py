...
class TestApplySCDType2(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.spark = SparkSession.builder.master("local[1]").appName("SCDTest").getOrCreate()
    ...
    def setUp(self):
        ...
        self.target_data = [
            (1, "Alice", "2024-01-01", "2024-01-01", "2024-01-01", "3000-01-01", True, "sk1"),
            (2, "Bob", "2024-01-01", "2024-01-01", "2024-01-01", "3000-01-01", True, "sk2")
        ]

        # Definir os schemas para os DataFrames
        self.source_schema = ["id", "name", "valid_from", "update_date"]
        self.target_schema = ["id", "name", "valid_from", "valid_to", "update_date", "is_current", "surrogate_key"]

        # Criar os DataFrames
        self.source_df = self.spark.createDataFrame(self.source_data, self.source_schema)
        self.target_df = self.spark.createDataFrame(self.target_data, self.target_schema)
    ...
    def test_apply_scd_type2(self):
        ...
        preserved_key = self.spark.sql("SELECT surrogate_key FROM result WHERE name = 'Alice'").collect()[0][0]
        ...
        new_key = self.spark.sql("SELECT surrogate_key FROM result WHERE name = 'Bob'").collect()[0][0]
        self.assertNotEqual(new_key, "sk2")  # Deveria ser uma chave diferente, pois 'sk2' já estava no target
    ...
    def test_edge_case_no_changes(self):
        ...
        preserved_key_bob = self.spark.sql("SELECT surrogate_key FROM result WHERE name = 'Bob'").collect()[0][0]
